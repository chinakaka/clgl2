...
const AdminRequestDetail: React.FC<AdminRequestDetailProps> = ({ user }) => {
    ...
    const handleStatusChange = async (status: RequestStatus) => {
        if (!id) return;
        await api.updateStatus(id, status, user);
        refresh();
    };
    ...
    const handleCompleteBooking = async () => {
        ...
        await api.completeBooking(id, user, {
            ...bookingDetails,
    ...
    const saveFiles = async () => {
        if (!id) return;
        setIsUploading(true);
        await api.updateBookingFiles(id, user, tempFiles);
        setIsUploading(false);
        setIsEditingFiles(false);
        refresh();
    };
    ...
    const handleFailBooking = async () => {
        if (!id) return;
        await api.failBooking(id, user, failReason);
        setActionMode('NONE');
        refresh();
    };
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Travelers List */}
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h3 className="text-lg font-bold text-gray-900 mb-4">Travelers / 出行人员详情</h3>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {request.data.travelers.map((traveler, index) => (
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{traveler.idType}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{traveler.idNumber}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{traveler.idExpiryDate || '-'}</td>
